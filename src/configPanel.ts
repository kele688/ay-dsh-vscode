...
/** 命令级授权规则的复合命令门禁：与前端 config-panel.js 的 COMMAND_METACHAR_RE、
 *  以及宿主 agent-host.mjs 的 SHELL_COMPOSITE_RE 保持同步——命令字段不允许包含
 *  shell 元字符（&& || ; | > < 反引号 $(），此类命令执行时按内核默认规则弹框授权。 */
...
export interface ProviderInfo {
  ...
  baseUrl: string;
  protocol?: string;
  /** dshProviders 元素的成员：该提供商**已被用户在配置时选中**的模型清单
   *  （每个模型可带默认参数或修改过的参数）＝ 配置结果。空数组＝尚未选中任何模型。
   *  提供商支持哪些模型不在此处落盘，永远由「获取模型」实时查询。 */
  models: ProviderModel[];
}

/** 首次打开时的**种子配置**（写进配置真源 dshProviders 的默认结果）。
 *  只预置两个默认已配置的提供商：DeepSeek (Official)（llm-deepseek 官方路由，其 API Key
 *  走环境变量 DEEPSEEK_API_KEY；不写入 llm-pi-ai.providers，是否写入由宿主侧决定）与
 *  Ollama（本地）。二者的 models 一律为空——厂家支持哪些模型必须靠「获取模型」实时
 *  查询、再由用户勾选（Official 还须先配好 API Key）。
 *  此后一切以 dshProviders（globalState 同名键）为准：对话面板的提供商与模型全部来自它。
 *  这两个种子项因"已接入"不出现在候选目录里，被删除后才回到候选。 */
const DEFAULT_PROVIDERS: ProviderInfo[] = [
  { id: "deepseek-official", name: "DeepSeek (Official)", type: "deepseek-official", baseUrl: "https://api.deepseek.com", models: [] },
  { id: "ollama", name: "Ollama (local)", type: "ollama", baseUrl: "http://localhost:11434/v1", models: [] },
];
...
export interface ConfigPanelDeps {
  ...
  /** 当前可见的工具集（配置页"审批规则"工具名建议；来自内核 ctx.tools.schemas）。 */
  queryToolCatalog: () => Promise<{ name: string; description?: string; fields?: string[] }[]>;
   ...
   *  带 `model` 时为**按需单模型查询**：只取该模型的默认容量参数（配置页仅在打开
   *  "尚未配置过"的模型编辑窗口时使用）；列表查询不带容量、保持轻量。 */
  discoverModels: (opts: { provider?: string; baseURL?: string; api?: string; apiKey?: string; model?: string }) => Promise<{ models: { id: string; name?: string; contextWindow?: number; maxTokens?: number; inputModalities?: string[] }[]; error?: string }>;
    ...
    try {
      ...
      panel.webview.postMessage({
        ...
        config: {
          ...
          // 必须下发：否则面板勾选"启用重申"保存后，下一次 sendConfig 会把勾选抹回未勾选
          enableReiteration: cfg.get<boolean>("enableReiteration") ?? false,
          autoApproveRules: cfg.get<{ match: string; action: string; commands?: string[] }[]>("autoApproveRules") ?? [],
          ...
          await Promise.all(
            loadProviders().map(async (p) => {
              ...
              try {
                ...
                if (!configured && p.id === "deepseek-official") {
  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    switch (msg.t) {
      ...
      case "providerSave": {
        ...
        // dshProviders 元素结构：models 必为数组，承载"用户选中/编辑过的模型"
        // （空数组 = 该提供商尚未选中任何模型 → 对话面板不提供其模型）。
        // 提供商支持哪些模型永远由「获取模型」实时查询，绝不写入配置。
        const selectedModels = Array.isArray(p.models)
          ? p.models.filter((m) => (typeof m === "string" && m !== "") || (m && typeof m.id === "string" && m.id !== ""))
          : [];
        const clean: ProviderInfo = { ...p, name: p.name.trim(), baseUrl: (p.baseUrl ?? "").trim(), models: selectedModels };
      ...
      case "fetchModelDefaults": {
        // 按需查询**单个**模型的默认容量参数：配置页仅在打开"尚未配置过"的模型编辑窗口时
        // 调用（已配置过的模型直接用配置值，不再查询）。列表查询不带容量，避免无用功。
        const defProviderId = typeof msg.providerId === "string" ? msg.providerId : "";
        const defModelId = typeof msg.modelId === "string" ? msg.modelId : "";
        if (!defProviderId || !defModelId) {
          panel.webview.postMessage({ t: "modelDefaults", model: defModelId, error: "missing provider/model" });
          break;
        }
        try {
          const disc = await deps.discoverModels({ provider: defProviderId, model: defModelId });
          const hit = Array.isArray(disc.models) ? disc.models.find((m) => m.id === defModelId) : undefined;
          panel.webview.postMessage({
            t: "modelDefaults",
            model: defModelId,
          ...
          panel.webview.postMessage({ t: "modelDefaults", model: defModelId, error: e instanceof Error ? e.message : String(e) });
        }
        break;
      }
      case "fetchModels": {
        ...
        // SSRF/密钥外带防线：baseUrl 必须是 http(s)，否则拒绝探活（apiKey 会经 Bearer 附加到该端点）
        if (baseUrl !== "") {
          let parsed: URL;
          try {
            parsed = new URL(baseUrl);
          } catch {
            panel.webview.postMessage({ t: "models", models: [], error: "invalid baseUrl" });
            break;
          }
          if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
            panel.webview.postMessage({ t: "models", models: [], error: "invalid baseUrl (only http/https)" });
            break;
          }
        }
      ...
      case "save": {
        ...
        try {
          // 普通配置项（Global）：**仅更新本次提供的字段**（未提供 = 不触碰），
          // 避免"保存运行环境组"把控制参数/日志字段重置为默认值。
          // （API Key / model / baseUrl 已改由 providerSave 处理，此处不再有死分支）
          const cfg = vscode.workspace.getConfiguration(CONFIG_NS);
          ...
          // nodePath 是安全敏感项（宿主将 spawn 该二进制）：已声明 scope:"machine"，
          // 必须写全局（用户）设置——工作区 .vscode/settings.json 不可覆盖，防注入
          // 恶意可执行文件在宿主启动时被加载（RCE）。
          if (typeof v.nodePath === "string") {
            await cfg.update("nodePath", v.nodePath.trim(), vscode.ConfigurationTarget.Global);
          }
          // 其余运行环境参数（defaultWorkspace/maxOutputChars）仍存**工作区设置**：
      ...
      case "toolCatalog": {
        // 配置页"添加规则"的工具名建议列表：宿主返回当前可见（裁剪后）的工具集
        try {
          const tools = await deps.queryToolCatalog();
          panel.webview.postMessage({ t: "toolCatalog", tools });
        } catch (e) {
          panel.webview.postMessage({ t: "toolCatalog", tools: [], error: e instanceof Error ? e.message : String(e) });
        }
        break;
      }
      case "savePermission": {
        // "权限审批"组"保存"：整体写回规则列表 {match, action, commands?}。**只落盘，不重启**
        // ——autoApproveRules 变更在扩展侧已单独排除自动重启（onDidChangeConfiguration），
        // 生效由功能组菜单"重启应用"（restartApply）统一触发。
        // 前端已逐行校验；此处再校验一次（纵深防御）：非法条目丢弃。
        const raw = (msg as { rules?: unknown }).rules;
        const COMMAND_MAX = 200;
        const rules = Array.isArray(raw)
          ? raw
              .filter((r): r is { match?: string; action?: string; commands?: unknown } => Boolean(r) && typeof r === "object")
              .map((r) => {
                ...
                if (Array.isArray(r.commands) && r.commands.length > 0) {
                  ...
                  for (const c of r.commands) {
                    ...
                    if (text === "" || text.length > COMMAND_MAX) continue;
                    // 复合命令门禁：含 shell 元字符的前缀一律丢弃（与 UI 文案
                    // permissionCommandNote / 前端 / 宿主三方一致），不降级为工具级。
                    if (COMMAND_METACHAR_RE.test(text)) continue;
                    const body = text.endsWith("*") ? text.slice(0, -1) : text;
                    if (body === "" || body.includes("*")) continue;
                    if (!prefixes.includes(body)) prefixes.push(body);
                  }
                  // 声明了命令但全部非法 → 丢弃整条规则（不降级为工具级，避免权限被放大）
              ...
              .filter(
                (r): r is { match: string; action: string; commands?: string[] } =>
                  Boolean(r && r.match && r.match.length <= 64 && /^[A-Za-z0-9*][A-Za-z0-9._*-]*$/.test(r.match))
              )
              .filter(
                (r, idx, arr) =>
                  arr.findIndex(
                    (x) => x.match === r.match && JSON.stringify(x.commands ?? []) === JSON.stringify(r.commands ?? []) && x.action === r.action
                  ) === idx
              )
          : [];
...
function renderHtml(webview: vscode.Webview, scriptUri: vscode.Uri, styleUri: vscode.Uri, zh: boolean): string {
  const L = {
    ...
    groupPermission: zh ? "授权规则" : "Approval rules",
    permissionKernelTitle: zh ? "内核默认规则" : "Kernel default rules",
    permissionKernelNote: zh ? "read / glob / grep 等读类工具不经过审批、永不弹框请求授权；<br>web_fetch / web_search 同样不弹框；<br>write / edit / pwsh / bash 在允许范围内直接执行，越界被沙箱拒绝后模型再用弹框申请提权。" : "read / glob / grep never go through approval and never prompt for authorization;<br>web_fetch / web_search never prompt either;<br>write / edit / pwsh / bash run directly within bounds; once the sandbox denies an out-of-bounds call, the model asks for escalation through a prompt.",
    permissionAutoTitle: zh ? "自动授权规则" : "Auto-approval rules",
    permissionAutoNote: zh ? "配置自定义授权规则后按顺序匹配命令字符串，命中即生效：允许 = 自动放行（不再弹框请求授权）；询问 = 弹框申请授权；拒绝 = 拒绝该工具调用。" : "Custom rules match the command string in order and the first match wins: Allow = pass without prompting; Ask = prompt for authorization; Deny = reject the tool call.",
    permissionCommandNote: zh ? "命令留空 = 工具级规则；为防复合命令绕过门禁，规则中命令字段不允许包含 \u201c&& ; | > < 反引号 $(\u201d 等字符——实际执行工具时，此类命令将按系统默认规则弹框申请授权。" : "An empty command means a tool-level rule; to keep composite commands from bypassing the gate, the command field must not contain \u201c&& ; | > < backtick $(\u201d — such commands fall back to the kernel default and prompt for authorization.",
...
<div class="cfg-layout">
  ...
  <section class="cfg-content">
    ...
    <div class="cfg-group" data-group="permission">
      <div class="cfg-pane active" data-pane="main">
        <h3 class="sub-title">${L.permissionKernelTitle}</h3>
        <p class="hint permission-note" style="margin-top:0">${L.permissionKernelNote}</p>
        <h3 class="sub-title">${L.permissionAutoTitle}</h3>
        <p class="hint permission-note" style="margin-top:0">${L.permissionAutoNote}</p>
